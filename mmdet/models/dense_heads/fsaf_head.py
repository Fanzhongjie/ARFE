...
class FSAFHead(RetinaHead):
    ...
    def loss(self,
        ...
        all_anchor_list = images_to_levels(concat_anchor_list,
                                           num_level_anchors)
        '''
        print('all_anchor_list: ', all_anchor_list)
        print('num_level_anchors: ', num_level_anchors)
        input()
        '''
        ...
        with torch.no_grad():
            ...
            # print(wl)
            f = open('./gt_assign.txt')
            line = list(f.readline().strip().split(' '))
            with open('./gt_assign.txt', 'w') as f:
                ws = [int(line[i]) + torch.sum(argmin==i).cpu().numpy() for i in range(self.anchor_generator.num_levels)]
