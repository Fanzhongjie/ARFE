...
class FPNMultiRF(nn.Module):
    ...
    def __init__(self,
        ...
        super(FPNMultiRF, self).__init__()
    ...
    def forward(self, inputs):
        ...
        ###
        class_map1 = F.relu(self.en_convs[0](laterals[2])).view(b, -1, 1)
        class_map2 = F.relu(self.en_convs[1](laterals[2])).view(b, 1, -1)
        ...
        for i in range(used_backbone_levels):
            laterals[i] = laterals[i] + \
                          F.interpolate(object_map, size=laterals[i].size()[2:]) + \
                          F.interpolate(class_map, size=laterals[i].size()[2:])
        ...
        ## take receptive field as hyper-addition
        # receptive field needed enlarges as the increasement of level for the object scale detected.
        # bins: 2^i * 2^i
        # for i in range(self.num_outs):
        #     outs[i] = outs[i] + F.interpolate(
        #                           F.adaptive_avg_pool2d(outs[i], output_size=[np.power(2, i), np.power(2, i)]), 
        #                           size=outs[i].size()[2:])
