...
class MultiBBoxHeadDualWS(BBoxHead):
    def __init__(self,
                 num_shared_convs=0,
                 num_shared_fcs=0,
                 num_cls_convs=0,
                 ...
                 *args,
                 **kwargs):
        super(MultiBBoxHeadDualWS, self).__init__(*args, **kwargs)
    ...
    def init_weights(self):
        super(MultiBBoxHeadDualWS, self).init_weights()
    ...
    def forward(self, x):
        ...
        ws_p = F.relu(self.compress_conv(x_out-ori_rois))
        ws_p = torch.softmax(self.pp_fc(ws_p.view(-1, 7*7)), dim=-1)
        
        ## weights
        # ws_ = F.relu(self.ws_conv(x))    # b, 3, h, w
        # ws_ = F.adaptive_avg_pool2d(ws_, output_size=[1, 1]).view(x.size(0), -1)
        # ws_ = torch.softmax(ws_, dim=-1)
        # p = ws_[:, 0:1]
        ...
        bbox_pred = self.fc_reg(x_reg) * ws_p[:, :1] + self.fc_reg(x_off) * ws_p[:, 1:] if self.with_reg else None
        
        return cls_score, bbox_pred


@HEADS.register_module()
class MultiRoIsBBoxHeadDualWS(MultiBBoxHeadDualWS):

    def __init__(self, fc_out_channels=1024, *args, **kwargs):
        super(MultiRoIsBBoxHeadDualWS, self).__init__(
            num_shared_convs=0,
            num_shared_fcs=2,
            num_cls_convs=0,
            num_cls_fcs=0,
